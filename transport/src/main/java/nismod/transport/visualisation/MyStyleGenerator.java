...
public class MyStyleGenerator {
    ...
    public static FeatureTypeStyle createFeatureTypeStyle(Classifier classifier,
        ...
        GeometryDescriptor geometryAttrType, int elseMode, double opacity, double width, Stroke defaultStroke)
        throws IllegalFilterException {
        //init nulls
        ...
        if (classifier instanceof RangedClassifier) {
            ...
            for (int i = 0; i < ranged.getSize(); i++) {
                ...
                Rule rule = createRuleRanged(ranged, expression, localMin, localMax,
                        geometryAttrType, i, elseMode, colors, opacity, width, defaultStroke);
                fts.addRule(rule);
            }
            ...
            for (int i = 0; i < explicit.getSize(); i++) {
                ...
                Rule rule = createRuleExplicit(explicit, expression, value, geometryAttrType, i,
                        elseMode, colors, opacity, width, defaultStroke);
                fts.addRule(rule);
            }
        } else {
    ...
    private static Symbolizer createSymbolizer(GeometryDescriptor geometryAttrType, Color color,
        double opacity, double width, Stroke defaultStroke) {
        Symbolizer symb;

        if (defaultStroke == null) {
            defaultStroke = sb.createStroke(color, 1.0f, opacity);
        }
        ...
        if ((geometryAttrType.getType().getBinding() == MultiPolygon.class)
        	...
        	symb = sb.createLineSymbolizer(color, width); //set the width of the line!
        } else if (geometryAttrType.getType().getBinding() == MultiLineString.class) {
        	symb = sb.createLineSymbolizer(color, width); //set the width of the line!
    ...
    private static Rule createRuleRanged(RangedClassifier classifier, Expression expression,
        ...
        int elseMode, Color[] colors, double opacity, double width, Stroke defaultStroke)
        throws IllegalFilterException {
        // 1.0 --> 1
